...
const createTripService = async (req) => {
  const {
    ...
    baggage,
    transport,
    reverse_trip_departure,
    reverse_trip_arrival,
    reverse_trip_points,
    reverse_trip_directions,
    reverse_trip_total_duration,
  } = req.body;
  const {
    _id,
    carrier_id,
  ...
  const trip_number_id = prevTripNumberId ? prevTripNumberId + 1 : 1;
  ...
  const prevDirectionNumberId =
    prevDirection?.length && prevDirection[0]?.direction_number_id;
  ...
  const updNewDirectionsReverse = await Promise.all(
    newDirectionsReverse?.map(async (direction, index) => {
      const updDirection = await Direction.findByIdAndUpdate(
        ...
        {
          direction_number_id:
            prevDirectionNumberId + 1 + index + updNewDirections?.length,
        },
        { new: true }
      );
  ...
  const newTripWithDirectionsIds = await Trip.findByIdAndUpdate(
    newTrip._id,
    {
      directions: newTrip?.directions?.map((i) => {
        const currentItemIdx = updNewDirections?.findIndex(
          (item) => item?.dfb_order === i?.dfb_order
        );
  ...
  const newTripReverseWithDirectionsIds = await Trip.findByIdAndUpdate(
    newTripReverse._id,
    {
      directions: newTripReverse?.directions?.map((i) => {
        const currentItemIdx = updNewDirectionsReverse?.findIndex(
          (item) => item?.dfb_order === i?.dfb_order
        );
