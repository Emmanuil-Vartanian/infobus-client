...
async function setDataToFrontEnd(res) {
  ...
  const passengersList = dataToFrontEnd?.reduce((prev, i, idx, arr) => {
    ...
    const isCurrentTripIn = prev?.find(trip => trip?.trip_id?.toString() === i?.trip_id?.toString());
    
    if (!isCurrentTripIn) {
      newPrev = prev.concat([{trip_id: i?.trip_id, departure_dates: [{date: i?.departure?.date, passengers_list: currentTripPassList}] }])
    }
    
    if (isCurrentTripIn) {
      // ===== check is current date in list of current trip
      const isCurrentTripDateIn = isCurrentTripIn?.departure_dates?.find(date => date?.date === i?.departure?.date);
      const isCurrentTripIndex = prev?.findIndex(trip => trip?.trip_id?.toString() === i?.trip_id?.toString());
      
      if (!isCurrentTripDateIn) {
        const newDepartureDates = isCurrentTripIn?.departure_dates?.concat([{date: i?.departure?.date, passengers_list: currentTripPassList}])
  
        newPrev[isCurrentTripIndex] = {trip_id: isCurrentTripIn?.trip_id, departure_dates: newDepartureDates}
      }

      if (isCurrentTripDateIn) {
        const newDepartureDates = isCurrentTripIn?.departure_dates?.map(date => {
          if (date?.date === i?.departure?.date) return {date: date?.date, passengers_list: date?.passengers_list?.concat(currentTripPassList)}
          return date
        })
    ...
    if (i?.type === "rt" && i?.departure_reverse?.date !== "free") {
      ...
      const isCurrentTripIn = newPrev?.find(trip => trip?.trip_id?.toString() === i?.reverse_trip_id?.toString());
      
      if (!isCurrentTripIn) {
        
        newPrev = newPrev.concat([{trip_id: i?.reverse_trip_id, departure_dates: [{date: i?.departure_reverse?.date, passengers_list: currentPassListReverse}] }])
      }
      
      if (isCurrentTripIn) {
        
        // ===== check is current date in list of current trip reverse
        const isCurrentTripDateIn = isCurrentTripIn?.departure_dates?.find(date => date?.date === i?.departure_reverse?.date);
        const isCurrentTripIndex = newPrev?.findIndex(trip => trip?.trip_id?.toString() === i?.reverse_trip_id?.toString());

        if (!isCurrentTripDateIn) {
        // ===== check is current date in list of current trip reverse
          const newDepartureDates = isCurrentTripIn?.departure_dates?.concat([{date: i?.departure_reverse?.date, passengers_list: currentPassListReverse}])
        ...
        if (isCurrentTripDateIn) {
          const newDepartureDates = isCurrentTripIn?.departure_dates?.map(date => {
            if (date?.date === i?.departure_reverse?.date) return {date: date?.date, passengers_list: date?.passengers_list?.concat(currentPassListReverse)}
            return date
          })
  ...
  const passengersListWithTripData = await Promise.all(
    passengersList?.map(async (tripItem) => {
      ...
      return {...tripItem, ...tripItemNewData, trip_info: trip}
    })
  )
