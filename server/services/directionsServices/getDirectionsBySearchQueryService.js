...
function setDataToFrontEnd(res) {
  ...
  const filteredRes = res?.filter((i) => {
    return String(i?.trip_info[0]?.departure?.end_date) >= today;
  });
  // return res
  const dataToFrontEnd = filteredRes?.map((i) => {
    ...
    const currentPointDeparture = points?.find(
      (item) => item?.point_order === i?.departure?.point_order
    );
    const newDeparture = {
      ...i?.departure,
    ...
    const currentPointArrival = points?.find(
      (item) => item?.point_order === i?.arrival?.point_order
    );
    const newArrival = {
      ...i?.arrival,
    ...
    const newSeats = seats?.map((i) => ({
      seat: i?.seat,
      booked: i?.booked_dates?.concat(i?.blocked_dates),
    }));
    const tripPricesListCurrentItem = prices?.list?.find(
      (item) => item?.dfb_order === i?.dfb_order
    );

    let newPrices = i?.prices;
    if (tripPricesListCurrentItem)
      newPrices = {
